...
# #             paragraphs = about_soup.find_all('p')
# #             about_content = ' '.join(p.get_text() for p in paragraphs).strip()[:3000]

# #         homepage_paragraphs = soup.find_all('p')
# #         homepage_text = ' '.join(p.get_text() for p in homepage_paragraphs).strip()[:3000]
...
def generate_pitch():
    try:
        ...
        # Step 1: Generate LaTeX code
        prompt_1 = f"""You are a LaTeX expert. Your task is to generate a complete LaTeX code for a pitch deck in Beamer format.
...
"""
        ...
        # Step 2: Debug the LaTeX code
        prompt_2 = f"""You are a LaTeX debugger. Fix any issues in the following LaTeX code and return only the corrected code:
